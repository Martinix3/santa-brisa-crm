...
const userFormSchema = z.object({
  ...
  if (data.role === "SalesRep") {
    if (data.monthlyTargetAccounts === undefined || data.monthlyTargetAccounts <= 0) {
      ctx.addIssue({ code: 'custom', message: "El objetivo mensual de cuentas es obligatorio y debe ser positivo para un Representante de Ventas.", path: ["monthlyTargetAccounts"] });
    }
    if (data.monthlyTargetVisits === undefined || data.monthlyTargetVisits <= 0) {
      ctx.addIssue({ code: 'custom', message: "El objetivo mensual de visitas es obligatorio y debe ser positivo para un Representante de Ventas.", path: ["monthlyTargetVisits"] });
    }
  }
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction asChild>
                                  <Button onClick={confirmDeleteUser} className={cn(buttonVariants({ variant: "destructive" }))}>Sí, eliminar de la app</Button>
                                </AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          )}
