                    ...
                    {fields.map((field, index) => (
                      <div key={field.id} className="flex flex-col gap-3 p-3 border rounded-md bg-secondary/20">
                        ...
                        <div className="flex items-end gap-2">
                           ...
                           <FormField control={form.control} name={`items.${index}.batchNumber`} render={({ field: bField }) => (<FormItem className="flex-grow"><FormLabel className="text-xs">Nº Lote Proveedor</FormLabel><FormControl><Input {...bField} value={bField.value ?? ''} disabled={isReadOnly} placeholder="Opcional" /></FormControl><FormMessage /></FormItem>)} />
                        </div>
                      </div>
                    ))}
