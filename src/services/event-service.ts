...
export const getEventsFS = async (): Promise<CrmEvent[]> => {
  const eventsCol = adminFirestore.collection(db, EVENTS_COLLECTION);
  const q = adminFirestore.query(eventsCol, adminFirestore.orderBy('startDate', 'desc'));
  const eventSnapshot = await adminFirestore.getDocs(q);
...
export const getEventByIdFS = async (id: string): Promise<CrmEvent | null> => {
  if (!id) return null;
  const eventDocRef = adminFirestore.doc(db, EVENTS_COLLECTION, id);
  const docSnap = await adminFirestore.getDoc(eventDocRef);
...
export const addEventFS = async (data: EventFormValues): Promise<string> => {
  ...
  const docRef = await adminFirestore.addDoc(adminFirestore.collection(db, EVENTS_COLLECTION), firestoreData);
  return docRef.id;
};
...
export const updateEventFS = async (id: string, data: EventFormValues): Promise<void> => {
  ...
  await adminFirestore.updateDoc(eventDocRef, firestoreData);
};

export const deleteEventFS = async (id: string): Promise<void> => {
  const eventDocRef = adminFirestore.doc(db, EVENTS_COLLECTION, id);
  await adminFirestore.deleteDoc(eventDocRef);
};


export const initializeMockEventsInFirestore = async (mockEventsData: CrmEvent[]) => {
    const eventsCol = adminFirestore.collection(db, EVENTS_COLLECTION);
    const snapshot = await adminFirestore.getDocs(adminFirestore.query(eventsCol));
    if (snapshot.empty && mockEventsData.length > 0) {
        ...
        mockEventsData.forEach(event => {
            ...
            const docRef = adminFirestore.doc(eventsCol);
