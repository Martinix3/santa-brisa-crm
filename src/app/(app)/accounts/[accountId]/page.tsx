...
import { getAccountDetailsAction, updateAccountAction } from "../actions";
...
export default function AccountDetailPage() {
  ...
  const canEditAccount = userRole === 'Admin' || userRole === 'Ventas';
  ...
  React.useEffect(() => {
    async function loadAccountData() {
      ...
      setIsLoading(true);
      try {
        const { account: foundAccount, allTeamMembers: fetchedMembers, allAccounts: fetchedAccounts, interactions, events } = await getAccountDetailsAction(accountId);
        ...
        setAllAccountsForValidation(fetchedAccounts);

        if (foundAccount.distributorId) {
            const foundDistributor = fetchedAccounts.find(acc => acc.id === foundAccount.distributorId);
        ...
        const combinedInteractions: InteractionItem[] = [
            ...interactions.map(o => ({ ...o, itemType: 'order' as const })),
  ...
  const salesRepAssigned = account.owner_user_id ? allTeamMembers.find(tm => tm.id === account.owner_user_id) : null;
    ...
    <div className="space-y-6" id="printable-account-details">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="shadow-subtle">
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="flex justify-between items-center">
                  ...
                  <strong className="font-medium text-right">{distributor ? distributor.name : 'Venta Directa SB'}</strong>
              </div>
              <Separator />
        ...
        <div className="md:col-span-2 space-y-6">
            <Card className="shadow-subtle">
                ...
                <CardContent className="space-y-4 text-sm">
                    <div>
                        ...
                        <p className="text-muted-foreground whitespace-pre-line pl-6">{formatAddress(account.billing_address)}</p>
                    </div>
                    <Separator />
                    <div>
                        ...
                        <p className="text-muted-foreground whitespace-pre-line pl-6">{formatAddress(account.shipping_address)}</p>
                    </div>
                </CardContent>
            </Card>
             <Card className="shadow-subtle">
            ...
            {(userRole === 'Admin' || userRole === 'Ventas') && account.internalNotes && (
              ...
              <Table>
                ...
                <TableBody>
                  {relatedInteractions.map(interaction => {
                    ...
                    const canRegisterResult = userRole === 'Admin' || (interaction.itemType === 'order' && ((userRole === 'Ventas' && teamMember?.name === (interaction as Order).salesRep) || (userRole === 'Clavadista' && (interaction as Order).clavadistaId === teamMember?.id)));
      ...
      {canEditAccount && account && (
        <AccountDialog
          ...
          onOpenChange={(open) => {
            ...
            if (!open) router.replace(`/accounts/${accountId}`, { scroll: false });
          }}
